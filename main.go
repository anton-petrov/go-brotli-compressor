...
func main() {
	...
	flag.StringVar(&alg, "a", "gzip", "algorithm (brotli, gzip")
	...
	if compress != "" {
		...
		if alg == "brotli" {
			...
			output_data, _ = CompressGzip(input_data)
		}
		ioutil.WriteFile(input+"."+alg, output_data, 0777)
	} else if decompress != "" {
		input = decompress
		input_data = ReadFile(input)
		...
		if alg == "brotli" {
			...
			output_data, _ = DecompressGzip(input_data)
		}
